                            ...
                            xccl_tl_team_t **tl_team)
{
    ...
    xccl_mrail_debug("Entering team create");
    ...
    xccl_mrail_debug("Exiting team create");
    *tl_team = &team->super;
    return XCCL_OK;
free_teams:
    team->n_teams -= 1;
...
{
    xccl_mrail_team_t *tl = ucs_derived_of(team, xccl_mrail_team_t);

    xccl_mrail_debug("Entering team destroy");
    free(tl);
    xccl_mrail_debug("Exiting team destroy");
                           ...
                           xccl_tl_coll_req_t **request, xccl_tl_team_t *tl_team)
{
    ...
    int                   last_buf_len;
    int                   len;
    xccl_mrail_debug("Entering collective init");
    ...
    for(req->n_reqs = 0; req->n_reqs < n_reqs; req->n_reqs++) {
        ...
        xccl_mrail_debug("len %d \n", len);
        team_args.buffer_info.dst_buffer = coll_args->buffer_info.dst_buffer +
                                           len*req->n_reqs;
        team_args.buffer_info.src_buffer = coll_args->buffer_info.src_buffer +
                                           len*req->n_reqs;
    ...
    (*request) = &req->super;
    xccl_mrail_debug("Exiting collective init");
...
static xccl_status_t xccl_mrail_collective_wait(xccl_tl_coll_req_t *request)
{
    ...
    xccl_mrail_debug("Entering collective wait");
    do {
...
xccl_status_t xccl_mrail_collective_finalize(xccl_tl_coll_req_t *request)
{
    ...
    xccl_mrail_debug("Entering collective finalize");
    global_status = XCCL_OK;
    for(i = 0; i < req->n_reqs; i++) {
    ...
    free(request);
    xccl_mrail_debug("Exiting collective finalize");
