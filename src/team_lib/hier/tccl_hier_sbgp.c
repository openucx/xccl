...
static inline int is_rank_local(int rank, tccl_hier_team_t *team, int local) {
    switch (local) {
    case LOCAL_NODE:
...
static inline tccl_status_t sbgp_create_local(sbgp_t *sbgp, int local) {
...
static tccl_status_t sbgp_create_node(sbgp_t *sbgp) {
    return sbgp_create_local(sbgp, LOCAL_NODE);
}

static tccl_status_t sbgp_create_socket(sbgp_t *sbgp) {
    return sbgp_create_local(sbgp, LOCAL_SOCKET);
}

static tccl_status_t sbgp_create_node_leaders(sbgp_t *sbgp) {
    tccl_hier_team_t *team = sbgp->hier_team;
    tccl_hier_context_t *ctx = tccl_derived_of(team->super.ctx,
                                               tccl_hier_context_t);
    ...
    for (i=0; i<comm_size; i++) {
        int ctx_rank = tccl_hier_team_rank2ctx(team, i);
...
static tccl_status_t sbgp_create_socket_leaders(sbgp_t *sbgp) {
    tccl_hier_team_t *team = sbgp->hier_team;
    tccl_hier_context_t *ctx = tccl_derived_of(team->super.ctx,
                                               tccl_hier_context_t);
    int comm_size     = team->super.oob.size;
...
tccl_status_t sbgp_create(tccl_hier_team_t *team, sbgp_type_t type) {
    ...
    case SBGP_SOCKET_LEADERS:
        ...
        break;
    default:
        status = TCCL_ERR_NOT_IMPLEMENTED;
        break;
    };
...
tccl_status_t sbgp_cleanup(sbgp_t *sbgp) {
    if (sbgp->rank_map) {
        free(sbgp->rank_map);
    }
    return TCCL_OK;
