                           ...
                           xccl_tl_team_t **team)
{
    ...
    if(mhba_team->transpose_hw_limitations){
        ...
        //todo check calc
        mhba_team->max_msg_size = (UINT64_MAX/MAX_OUTSTANDING_OPS)/
            (node_size*mhba_team->size);
    }

    storage_size = (MHBA_CTRL_SIZE+ (2*MHBA_DATA_SIZE)) * node_size * MAX_OUTSTANDING_OPS +
    ...
    if (mhba_team->node.asr_rank == node->group_rank) {
        ...
        tmpnam(bcast_data.sock_path); //todo make sure security warning mentioned in tempnam API
    }
    ...
    for(i=0;i<MAX_OUTSTANDING_OPS;i++){
        ...
        op->ctrl = mhba_team->node.storage + MHBA_CTRL_SIZE*MAX_OUTSTANDING_OPS
                +MHBA_CTRL_SIZE*node_size*i;
        op->my_ctrl = (void *) ((ptrdiff_t) op->ctrl + node->group_rank * MHBA_CTRL_SIZE);
        memset(op->my_ctrl, 0, MHBA_CTRL_SIZE);
        *((int*)op->my_ctrl) = -1; // because sequence number begin from 0
        op->send_umr_data = (void*)((ptrdiff_t)mhba_team->node.storage +
        ...
        op->recv_umr_data = (void*)((ptrdiff_t)op->send_umr_data +
    ...
    if (mhba_team->node.asr_rank == node->group_rank) {
        ...
        local_data_size = (net_size*sizeof(uint32_t))+sizeof(uint32_t)+2*sizeof(uint32_t)+sizeof(void*);
        ...
        *((uint64_t*)&local_data[net_size+2]) = (uint64_t)(uintptr_t)mhba_team->net.ctrl_mr->addr;

        mhba_team->net.remote_ctrl = calloc(sizeof(*mhba_team->net.remote_ctrl), net_size);
        ...
        for (i=0; i<net_size; i++) {
            ...
            xccl_mhba_qp_connect(mhba_team->net.qps[i], remote_data[net->group_rank],
        ...
        mhba_team->work_completion = (struct ibv_wc*) malloc(sizeof(struct ibv_wc)*net_size);
