...
void *do_sendmsg(void *ptr)
{
    ...
    *p                  = conn->fd;
    xccl_mhba_debug("sendmsg: %d, msg.msg_iovlen=%d", conn->fd,
                    (int)msg.msg_iovlen);
...
static xccl_status_t do_recvmsg(int sock, int *shared_cmd_fd,
                                uint32_t *shared_pd_handle)
{
    ...
    p = (int *)CMSG_DATA(cmsghdr);
    xccl_mhba_debug("recvmsg: %d, nbytes=%d, handle1=%d", *p, (int)nbytes,
                    handles[0]);
...
static xccl_status_t client_recv_data(int        *shared_cmd_fd,
                                      uint32_t   *shared_pd_handle,
                                      ...
                                      const char *sock_path)
{
    ...
    while (num_of_curr_connections < num_of_connections) {
        ...
        if (connection[num_of_curr_connections].sock != -1) {
            ...
            pthread_create(&thread[num_of_curr_connections], 0, do_sendmsg,
                           (void *)&connection[num_of_curr_connections]);
                                     ...
                                     const char       *sock_path)
{
    ...
    if (node->asr_rank != node->sbgp->group_rank) {
        ...
        node->shared_pd = ibv_import_pd(node->shared_ctx,
                                        shared_pd_handle);
        if (!node->shared_pd) {
