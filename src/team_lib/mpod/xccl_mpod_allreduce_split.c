...
#define INTRA_POD_ALLGATHER_QUEUED  (3)
#define INTRA_POD_ALLGATHER_INITIATED  (4)
#define INTRA_POD_ALLGATHER_COMPLETED  (5)
...
static xccl_status_t allreduce_test(xccl_mpod_coll_req_t *req)
{
    ...
    for (int i = 0; i < req->num_chunks; i++) {
        ...
        case INTRA_POD_ALLREDUCE_INITIATED:
            ...
            if (status == XCCL_OK) {
                if (req->team->num_pods == 1) {
                    req->chunks[i].phase_id = INTRA_POD_ALLGATHER_COMPLETED;
                } else {
        ...
        case INTER_POD_ALLREDUCE_INITIATED:
            ...
            if (status == XCCL_OK) {
                req->chunks[i].phase_id = INTRA_POD_ALLGATHER_QUEUED;
            }
            break;

        case INTRA_POD_ALLGATHER_QUEUED:
            if (i == 0 || req->chunks[i-1].phase_id > req->chunks[i].phase_id) {
                status = xccl_mpod_nccl_req_post(&req->chunks[i].real_req.nccl[1]);
                xccl_mpod_err_pop(status, fn_fail);

                req->chunks[i].phase_id = INTRA_POD_ALLGATHER_INITIATED;
            }
            break;
        ...
        case INTRA_POD_ALLGATHER_COMPLETED:
            break;
        }
    ...
    for (int i = 0; i < req->num_chunks; i++) {
        if (req->chunks[i].phase_id == INTRA_POD_ALLGATHER_COMPLETED) {
...
xccl_status_t xccl_mpod_allreduce_init_split(xccl_mpod_coll_req_t *req)
{
    ...
    for (int i = 0; i < req->num_chunks; i++) {
        ...
        nccl_coll_args.coll_type = XCCL_ALLGATHER;
